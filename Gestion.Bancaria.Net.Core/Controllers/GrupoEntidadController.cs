...
namespace Gestion.Bancaria.Net.Core.Controllers.GrupoController
{
	...
	[ApiController]
	public class GrupoEntidadController : ControllerBase
	{
		...
		public GrupoEntidadController(EntitiesGestionBancaria context)
		{
			ctxModel = context;
		}
		...
		{
			IQueryable<EntidadesAsociadasEntity> Consulta = this.ctxModel.EntidadesAsociadas;
			...
			return this.Ok(Consulta.Select(b => new EntidadesAsociadas(b)));
		}

		[HttpPost]
		public IActionResult Create([FromBody] GrupoEntidadEntity GrupoEntidadEntityParam)
		{
			string json = "";
			json = JsonConvert.SerializeObject(GrupoEntidadEntityParam);
			...
			try
			{
				ctxModel.GrupoEntidad.Add(GrupoEntidadEntityParam);
				ctxModel.SaveChanges();
				ResultBool = true;
				MensajeString = "Empresa Asociada al grupo.";
			}
			catch (Exception ErroresControlado)
			{
				MensajeString = ErroresControlado.GetBaseException().Message;
